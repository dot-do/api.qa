...
describe('data: URI hygiene (ax-17j): inline content PASSes, smuggled credentials still FAIL', () => {
  ...
  it('(e) a padded FAKE data:image/png (≥512B, no PNG magic) → envelope-hygiene FAILs (residual closed)', async () => {
    const structured = { count: 3, blob: `data:image/png;base64,${paddedFake}` }
    const { checks } = await judge({ toolCallResult: defaultResult({ structuredContent: structured }) })
    expect(verdictOf(checks, 'mcp-ui-envelope-hygiene'), detailOf(checks, 'mcp-ui-envelope-hygiene')).toBe('fail')
  })

  it('(e) padded FAKE data:image/jpeg and data:application/pdf (≥512B, wrong magic) → envelope-hygiene FAILs', async () => {
    for (const blob of [`data:image/jpeg;base64,${paddedFake}`, `data:application/pdf;base64,${paddedFake}`]) {
      const { checks } = await judge({ toolCallResult: defaultResult({ structuredContent: { count: 3, blob } }) })
      expect(verdictOf(checks, 'mcp-ui-envelope-hygiene'), `${blob.slice(0, 40)}: ${detailOf(checks, 'mcp-ui-envelope-hygiene')}`).toBe('fail')
    }
  })

  it('(e) data:application/octet-stream (dropped from exempt set) opaque blob → envelope-hygiene FAILs', async () => {
    const structured = { count: 3, blob: `data:application/octet-stream;base64,${paddedFake}` }
    const { checks } = await judge({ toolCallResult: defaultResult({ structuredContent: structured }) })
    expect(verdictOf(checks, 'mcp-ui-envelope-hygiene'), detailOf(checks, 'mcp-ui-envelope-hygiene')).toBe('fail')
  })

  it('(e) data:image/svg+xml carrying OPAQUE base64 (not real svg) → envelope-hygiene FAILs', async () => {
    const structured = { count: 3, blob: `data:image/svg+xml;base64,${opaqueSvg}` }
    const { checks } = await judge({ toolCallResult: defaultResult({ structuredContent: structured }) })
    expect(verdictOf(checks, 'mcp-ui-envelope-hygiene'), detailOf(checks, 'mcp-ui-envelope-hygiene')).toBe('fail')
  })

  it('(e) REAL magic-matched media (jpeg FFD8FF, pdf %PDF, woff2 wOF2) → envelope-hygiene PASSes', async () => {
    const structured = {
      count: 3,
